...
public class TestAddBookUsecase {
    ...
    public void testAddTextBookSuccess() throws Exception {
        ...
        verify(mockOutputBoundary).presentResult(any(AddBookResponseData.class)); // Kiểm tra xem kết quả được trình bày đúng hay không
    }
    ...
    public void testAddReferenceBookSuccess() throws Exception {
        ...
        // Assert
        verify(mockRepository).addBook(any(ReferenceBook.class)); // Kiểm tra xem sách có được thêm vào repository hay không
        verify(mockOutputBoundary).presentResult(any(AddBookResponseData.class)); // Kiểm tra xem kết quả được trình bày đúng hay không
    }
