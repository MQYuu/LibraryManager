...
public class EditBookServiceTest {
    ...
    public void testEditBookSuccess() {
        ...
        EditBookService editBookService = new EditBookService(mockDBBoundary, mockOutputBoundary);

        // Dữ liệu yêu cầu chỉnh sửa sách
        EditBookRequestData requestData = new EditBookRequestData("T001", "2024-11-20", 100.0, 10, "Publisher A", "New", 10.0);

        // Giả lập hành động chỉnh sửa sách thành công
        doNothing().when(mockDBBoundary).updateBook(requestData);
        ...
        verify(mockDBBoundary).updateBook(requestData);
    ...
    public void testEditBookFailure() {
        ...
        doThrow(new RuntimeException("Error updating book")).when(mockDBBoundary).updateBook(requestData);
    ...
    public void testEditBookWithInvalidData() {
        ...
        EditBookRequestData requestData = new EditBookRequestData("", "2024-11-20", 100.0, 10, "Publisher A", "New", 10.0);

        // Act: Gọi phương thức chỉnh sửa sách
        editBookService.editBook(requestData);

        // Assert: Kiểm tra xem phương thức chỉnh sửa không được gọi nếu dữ liệu không hợp lệ
        verify(mockDBBoundary, never()).updateBook(any(EditBookRequestData.class));
